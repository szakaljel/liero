...
class Input:
    ...
    def draw(self, surface):
        ...
        com=self.prompt+self.value+self.rest
        text = self.font.render(com, 1, self.color)
        (w,h)=self.font.size("w"*self.maxlength)
        r=Rect(self.x-self.margin,self.y-self.margin,w+2*self.margin,h+2*self.margin)
        pygame.draw.rect(surface,self.colorRect,r)
